...
usage:
	...
	pr_msg("\n"
	       ...
	       "  --enable-object-storage       enable object storage for lazy restore\n"
	       "  --object-storage-upload       upload checkpoint to object storage on dump\n"
	       "  --object-storage-endpoint-url URL\n"
	       "                                S3-compatible endpoint URL\n"
	       "  --object-storage-bucket NAME  bucket name\n"
	       "  --object-storage-object-prefix PREFIX\n"
	       "                                object key prefix\n"
	       "  --object-storage-path-style   use path-style URLs (for MinIO)\n"
	       "  --aws-access-key KEY          AWS access key ID\n"
	       "  --aws-secret-key KEY          AWS secret access key\n"
	       "  --aws-region REGION           AWS region (e.g., us-west-2)\n"
	       "  --express-one-zone            use S3 Express One Zone\n"
	       "\n"
	       "Lazy restore options:\n"
	       "  --no-semi-sync-iov            disable semi-synchronous IOV fetch\n"
	       "                                (fall back to page-by-page fetch for ablation)\n"
	       "  --object-storage-parallel-xfer\n"
	       "                                enable parallel xfer path (worker pool installs\n"
	       "                                pages directly via UFFDIO_COPY) for object storage\n"
	       ...
	       "  --compress                    compress pages-*.img with zstd seekable format\n"
	       "                                (dump: produce compressed images; restore: auto-detected)\n"
